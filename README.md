...
- **参数设置**：
  - **最大迭代轮次**：设置优化迭代的上限（1-20 轮）
  - **验证阈值**：设置验证改写的严格程度（0-100%）
    - 50%：宽松模式，解决一半 badcase 即可接受
    - 100%：严格模式，必须解决所有 badcase（默认）
- **实时日志**：实时显示优化过程，包括预测、分析、改写、验证等各阶段信息
- **优化结果**：优化完成后显示准确率指标、变化趋势图和优化后的 prompt

### 2. 准确率变化趋势

系统会显示完整的准确率变化折线图，包含所有迭代轮次的数据点，清晰展示优化过程中的准确率变化趋势。
...
- 🤖 五个 Agent 协作优化：
  ...
  4. **改写 Agent**: 根据分析结果和历史经验改写 Prompt
  5. **验证 Agent**: 验证改写后的 prompt 是否能解决目标 badcase（必须解决所有 badcase 才接受）
...
判断是否有错误？
  ...
  [改写 Agent] → 根据分析建议和历史经验改写 prompt
      ↓
  [验证 Agent] → 使用新 prompt 对 badcase 进行验证
      ↓
  判断验证结果？
      ├─ 接受（解决所有 badcase）→ 判断是否继续迭代
      │   ├─ 继续迭代（未达上限）→ 增加迭代次数 → 回到预测 Agent
      │   └─ 达到最大迭代次数 → [最终验证] → 使用最终优化后的 prompt 进行最后一次预测 → 结束
      └─ 拒绝（未解决所有 badcase）→ 回滚 prompt → 重新分析 → 回到改写 Agent
```
...
4. **改写阶段**：结合分析建议和历史优化经验，生成新的优化 prompt
5. **验证阶段**：使用改写后的 prompt 对上一轮的 badcase 进行验证
   - **接受条件**：必须解决所有 badcase（100%）
   - **拒绝处理**：如果未解决所有 badcase，回滚到改写前的 prompt，重新分析并改写
6. **终止条件**：
   - 所有预测都正确：直接结束，返回当前优化后的 prompt
   - 达到最大迭代次数：会执行一次 `final_predict` 节点，使用最终改写后的 prompt 进行验证，然后结束
...
3. **接受标准**：必须解决所有 badcase（100%），否则拒绝此次改写
4. **拒绝处理**：
   - 回滚到改写前的 prompt
   - 将验证反馈信息传递给分析 Agent
   - 重新分析错误案例（结合验证反馈）
   - 重新改写 prompt
5. **优势**：避免无效的改写进入下一轮，确保每轮迭代都基于有效的 prompt
...
1. 需要有效的 OpenAI API Key（或 Azure OpenAI 配置）
...
7. 达到最大迭代次数时，系统会对最终优化后的 prompt 执行一次验证预测（`final_predict`），确保返回的是最终版本的准确结果
8. 系统内部设置了足够的递归限制，避免在迭代过程中出现递归限制错误
9. **验证机制**：每次改写后必须通过验证（解决所有 badcase）才会进入下一轮，否则会回滚并重新改写
10. **智能选择**：如果最终准确率未达到 100%，系统会自动返回历史最高准确率对应的 prompt，并在前端标注来源
11. **历史记录**：每轮迭代的 history 记录的是该轮 analyze 时的 prompt 和结果，如果发生回滚，最终被接受的 prompt 会在下一轮的 history 中记录
